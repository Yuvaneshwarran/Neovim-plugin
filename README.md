...
* **Go Language Support**: Integrated `gofmt`, `goimports`, and `golangci-lint` for Go development workflows.
...
* **Config Hot Reload**: Reload Neovim configuration without restarting.
...
* **Go Tools** (for Go development):
  * **gofmt**: Code formatting for Go (included with Go installation)
  * **goimports**: Automatically manages Go imports
  * **golangci-lint**: Advanced linting tool for Go
...
2. Open Neovim and run the following command to install plugins:
   ...
   :Lazy sync
   ```
3. Install Go development tools (if needed):
...
| Shortcut           | Action           | Description                                               |
| ------------------ | ---------------- | --------------------------------------------------------- |
| [Auto on save]     | Format with gofmt | Automatically formats Go code when saving                |
| [Auto on save]     | Run goimports    | Automatically organizes imports when saving              |
| `<leader>gl`       | Run golangci-lint | Run linter on current Go file or project                |

### Config Management

| Shortcut       | Action            | Description                                           |
| -------------- | ----------------- | ----------------------------------------------------- |
| `<leader>rs`   | Reload Config     | Reload Neovim configuration without restarting        |
...
This configuration comes with comprehensive Go language support:

### Features:

1. **Automatic Formatting**: Files are formatted with `gofmt` on save.
2. **Import Management**: `goimports` automatically organizes imports on save.
3. **Advanced Linting**: Integration with `golangci-lint` provides robust code quality checks.

### Configuration:

The Go tools are configured through the `none-ls.lua` plugin file. The setup includes:

```lua
-- Example configuration in none-ls.lua
